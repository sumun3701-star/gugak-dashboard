...
### Gemini API 키 — AI 기능 (선택)

1. <https://aistudio.google.com/apikey> 에서 API 키 발급 (무료 등급 있음)
2. 대시보드 **설정 → Gemini API 키** 에 붙여넣고 저장
3. 켜지는 기능:
   - **스크랩 › ✨ AI 정리**: URL 넣고 누르면 본문을 읽어 제목·요약·태그·카테고리를 자동으로 채움 (본문 추출은 로컬 서버 필요)
...
| 스크랩 (확장) | — | 기사·블로그 URL + 제목·메모·태그 저장, 읽음 체크, 태그/읽음/텍스트 필터. **네이버 블로그식 2단계 카테고리 트리**(작업물 게시판과 동일 — 상위→하위, 트리 사이드바, 접기/건수/순서이동/이름변경·삭제, 그룹 보기). 로컬 서버 실행 시 "정보 가져오기"로 og:title·설명 자동 채움 |
...
js/cattree.js        2단계 카테고리 트리 공용 모듈 (작업물 게시판·스크랩)
